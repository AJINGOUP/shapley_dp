...
class Adult(Dataset):
    ...
    def __init__(
        ...
        train: bool = True,
        sensitive_attributes: Union[str, Tuple[str, ...]] = (),
        download: bool = False,
        ...
        if not self.files_dir.exists():
            if not download:
                raise RuntimeError("Dataset not found. Download it by passing download=True.")
        ...
        data.drop(data.columns[list(self.__sensitive_colum_indices)], axis=1, inplace=True)
        self.data = torch.tensor(data.values.astype(np.float32), dtype=torch.get_default_dtype())
    ...
    def _preprocess_features(
        ...
        categorical_cols = [col for col, vals in self.variables.items() if vals is not None]
        ...
        continuous_cols = [col for col, vals in self.variables.items() if vals is None]
...
@register_dataset_factory(name="adult")
def dataset_factory(val_ratio: float = 0.2) -> DatasetPack:
    train_dataset = Adult(DATASET_DIR, download=True, train=True)
    test_dataset = Adult(DATASET_DIR, download=True, train=False)
    train_dataset, val_dataset = split_dataset(train_dataset, val_ratio)
    infos = DatasetInfo(name="adult", num_classes=2, vector_size=train_dataset[0][0].size(0))
